...
.budgie-panel {
  ...
  .alert { color: $inverted_accent_label_color; }
}
...
.budgie-menu {
  ...
  row { // reset background
    &, &.activatable {
      ...
      // FIXME: enlarge vertical spacing with icon's padding
...
UnityDecoration {
  ...
  -UnityDecoration-active-shadow-color: rgba(0, 0, 0, 0.34);
  -UnityDecoration-active-shadow-radius: 18px;
  -UnityDecoration-inactive-shadow-color: rgba(0, 0, 0, 0.12);
...
// FIXME: ignored box-shadow and wrong pseudo-class for toggled-state
.libreoffice-toolbar {
  button {
    // active <- checked
